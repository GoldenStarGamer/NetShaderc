...
namespace NetShaderc
{
	public class Compiler : IDisposable
	{
		...
		public Result Compile(string source, CompileType type, ShaderKind kind, string file = "", string entry = "main", Opt? opt = null)
		{
			if (disp) throw new ObjectDisposedException($"Compiler {nameof(Compile)} has been disposed");
			switch (type)
			{
				case CompileType.CodeToSPIRV:
					return new(Shaderc.shaderc_compile_into_spv(handle, source, (nuint)source.Length, kind, file, entry, (opt == null ? nint.Zero : opt)));
